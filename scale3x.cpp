...
static inline void scale3x_32_def_border(DWORD* __restrict__ dst, const DWORD* __restrict__ src0, const DWORD* __restrict__ src1, const DWORD* __restrict__ src2, unsigned count)
{
	assert(count >= 2);
...
static inline void scale3x_32_def_center(DWORD* __restrict__ dst, const DWORD* __restrict__ src0, const DWORD* __restrict__ src1, const DWORD* __restrict__ src2, unsigned count)
{
	assert(count >= 2);
 ...
 * \param dst2 Third destination row, triple length in pixels.
 */
void scale3x_32_def(DWORD* dst0, DWORD* dst1, DWORD* dst2, const DWORD* src0, const DWORD* src1, const DWORD* src2, unsigned count)
{
	assert(count >= 2);
...
void Scale3x16(WORD *dstPtr,WORD *srcPtr,int width,int height,int pitch)
{
  ...
  WORD *dst2 = dst1 + pitch;
    
  WORD *src0 = srcPtr;
  WORD *src1 = src0 + width;
  WORD *src2 = src1 + width;

  scale3x_16_def(dst0, dst1, dst2, src0, src0, src1, width);
  ...
  while(count) 
  {
     ...
     dst1 += pitch*3;
     dst2 += pitch*3;
     scale3x_16_def(dst0, dst1, dst2, src0, src1, src2, width);
     src0 = src1;
     src1 = src2;
     src2 += width;
     --count;
...
void Scale3x32(DWORD *dstPtr,DWORD *srcPtr,int width,int height,int pitch)
{
  ...
  DWORD *dst2 = dst1 + pitch;
    
  DWORD *src0 = srcPtr;
  DWORD *src1 = src0 + width;
  DWORD *src2 = src1 + width;

  scale3x_32_def(dst0, dst1, dst2, src0, src0, src1, width);
  ...
  while(count) 
  {
     ...
     dst1 += pitch*3;
     dst2 += pitch*3;
     scale3x_32_def(dst0, dst1, dst2, src0, src1, src2, width);
     src0 = src1;
     src1 = src2;
     src2 += width;
     --count;
