...
void gb_system::draw_line_tile_GBC()
{
   ...
   if(((memory[0xFF40] & 0x01) || cartridge->header.CGB) && (video_enable&VID_EN_BG))
   {
      while(x < 160) 
      { 
         if(attrs & 0x40) 
   ...
   } else 
   {
      ...
      unsigned short c = (cartridge->header.CGB?GBC_BGP[0]:GBC_BGP[BGP[0]]);
      if(gfx_bit_count == 16)
         fill_line16(((WORD*)gfx_buffer)+((y<<7)+(y<<5)),palette.gfxPal16[c]|(palette.gfxPal16[c]<<16),160);
      else
         fill_line32(((DWORD*)gfx_buffer)+((y<<7)+(y<<5)),palette.gfxPal32[c],160);     
...
void gb_system::draw_sprite_tile(int tile, int x,int y,int t, int flags,int size,int spriteNumber)
{
   ...
   if((gbc_mode && cartridge->header.CGB) && (flags & 0x08))
   {
      a = bank1[address++];
      b = bank1[address++];
   } else 
   ...
   for(int xx = 0; xx < 8; xx++) 
   {
      ...
      if((cartridge->header.CGB && gbc_mode && (memory[0xFF40]&1)) || !gbc_mode || (gbc_mode && !cartridge->header.CGB))
      {
         if(prio) 
         {
