...
void gb_system::crdtype(byte value,byte romsize,byte ramsize)
{
   ...
   mbc->setMemoryRead(MEMORY_DEFAULT);
   mbc->setMemoryWrite(MEMORY_DEFAULT);
   ...
   case 0x01: //"MBC1"
       rom->battery = false;
       rom->bankType = MBC1;
       mbc->setMemoryWrite(MEMORY_MBC1);
   break;
   ...
   case 0x05: //"MBC2"
       rom->battery = false;
       rom->bankType = MBC2;
       mbc->setMemoryWrite(MEMORY_MBC2);
   break;
   ...
   case 0x0B: //"MMM01"
       rom->battery = false;
       rom->bankType = MMM01;
       mbc->setMemoryWrite(MEMORY_MMM01);
   break;
   ...
   case 0x0F: //"MBC3+TIMER+BATTERY"
       ...
       rom->bankType = MBC3;
       rom->RTC = true;
       mbc->setMemoryWrite(MEMORY_MBC3);
       mbc->setMemoryRead(MEMORY_MBC3);
   break;
   ...
   case 0x19: //"MBC5"
       rom->battery = false;
       rom->bankType = MBC5;
       mbc->setMemoryWrite(MEMORY_MBC5);
   break;
   ...
   case 0x22: //Kirby's Tilt'n'Tumble
       rom->battery = true;
       rom->bankType = MBC7;
       mbc->setMemoryWrite(MEMORY_MBC7);
       mbc->setMemoryRead(MEMORY_MBC7);
   break;
   ...
   case 0xFC: //"POCKET CAMERA"
       rom->battery = true;
       rom->bankType = Camera;
       mbc->setMemoryWrite(MEMORY_CAMERA);
       mbc->setMemoryRead(MEMORY_CAMERA);
   break;

   case 0xFD: //"Bandai TAMA5"
       rom->battery = true;
       rom->bankType = TAMA5;
       mbc->setMemoryWrite(MEMORY_TAMA5);
       mbc->setMemoryRead(MEMORY_TAMA5);
   break;

   case 0xFE: //"Hudson HuC-3"
       rom->battery = true;
       rom->bankType = HuC3;
       mbc->setMemoryWrite(MEMORY_HUC3);
       mbc->setMemoryRead(MEMORY_HUC3);
   break;
...
int gb_system::process_rom_info(byte *rominfo,byte *logo1, byte *logo2)
{
   ...
   switch(options->unl_compat_mode) {
	...
	if (useNiutoude) {
		...
		rom->battery = true;
		rom->bankType = MBC5;
		mbc->setMemoryWrite(MEMORY_NIUTOUDE);
		...
		rom->carttype=0x1B;
   		...
   		rom->battery = true;
		rom->bankType = MBC5;
        mbc->setMemoryRead(MEMORY_SINTAX);
        mbc->setMemoryWrite(MEMORY_SINTAX);
   		rom->ROMsize=07; // assumption for now
   ...
   {
      rom->ROMsize = 7;
      mbc->setMemoryWrite(MEMORY_POKE);
   }
   else   
   // SGB Pack
   ...
   {
      mbc->setMemoryWrite(MEMORY_MK12);
   }
   else
   // Gameboy Camera
...
bool gb_system::loadrom_zip(const wchar_t* filename)
{
  ...
  char buffer_file_name[300];
   ...
   {
      mbc->setMemoryWrite(MEMORY_8IN1);
      rom->ROMsize = 4;
   } else
