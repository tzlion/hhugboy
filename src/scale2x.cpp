...
*/
/*
 * You are allowed to use this code in your program with these conditions:
...
static inline void scale2x_32_def_single(DWORD *dst, const DWORD *src0, const DWORD* src1, const DWORD* src2, unsigned int count) 
{
	/* first pixel */
	dst[0] = src1[0];
	if (src1[1] == src0[0] && src2[0] != src0[0])
...
static void internal_scale2x_32_mmx_single(DWORD* dst, const DWORD* src0, const DWORD* src1, const DWORD* src2, unsigned count) {
  /* always do the first and last run */
  count -= 2*2;
...
void Scale2x(WORD *dstPtr,WORD *srcPtr,int width,int height,int pitch)
{
  ...
  {
    scale2x_16_def(dst0, dst1, src0, src0, src1, width);
    ...
    while(count) {
      dst0 += pitch<<1;
      dst1 += pitch<<1;
      scale2x_16_def(dst0, dst1, src0, src1, src2, width);
      src0 = src1;
      src1 = src2;
      src2 += width;
      --count;
    ...
    dst0 += pitch<<1;
    dst1 += pitch<<1;
    scale2x_16_def(dst0, dst1, src0, src1, src1, width);
  }
}
...
void Scale2x(DWORD *dstPtr,DWORD *srcPtr,int width,int height,int pitch)
{
  ...
  {
     scale2x_32_def(dst0, dst1, src0, src0, src1, width);
     ...
     while(count) 
     {
        dst0 += pitch<<1;
        dst1 += pitch<<1;
        scale2x_32_def(dst0, dst1, src0, src1, src2, width);
        src0 = src1;
        src1 = src2;
        src2 += width;
        --count;
