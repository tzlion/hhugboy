...
void MbcUnlPokeAct::writeMemory(unsigned short address, register byte data) {
    if (address >= 0x2000 && address <= 0x3000) {
        if (data >= 0x60 && data < 0x80) {
            data -= 0x40; // the last 0x20 banks are accessed by bank numbers 0x40 greater than their actual ones
        } else if (data >= 0x20 && data <= 0x3f) {
            data += 0x40; // but the actual bank numbers for the above banks return nothing (at least on a 1mb cart)
            // so just flip the mapping here to fake that behaviour later
        }
    }
...
byte MbcUnlPokeAct::readMemory(unsigned short address) {
    if (address >= 0x4000 && address <= 0x7fff) {
        if (rom_bank >= 0x40 && rom_bank <= 0x7f) {
            return 0xff; // reads on these banks give either all ff or alternating 4 bytes of 00/ff depending on the cart
            // can't imagine any game would rely on this behaviour since it's not consistent but here it is anyway
        } else if (rom_bank >= 0x80) {
